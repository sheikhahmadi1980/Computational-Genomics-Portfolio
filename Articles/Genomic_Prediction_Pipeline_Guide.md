...
## Phase 2: From Individuals to a Family - The Genomic Relationship Matrix (GRM)

Now that we have a clean dataset, we move from treating individuals as isolated data points to understanding them as members of a single, complex family. The Genomic Relationship Matrix (GRM) is the powerful tool that allows us to do this.

**The "Why":** Think of the GRM as a highly detailed, quantitative family tree built not from historical records, but directly from DNA. For any two individuals, it calculates a single number that represents how genetically similar they are based on hundreds of thousands of SNPs. This matrix is the mathematical foundation for almost all advanced quantitative genetics, including heritability estimation and genomic prediction. It allows our models to understand, for instance, that two full siblings should have more similar outcomes for a trait than two distant cousins.

**The "How":** Using GCTA, creating the GRM is a straightforward but computationally intensive step. The command essentially takes our clean dataset from Phase 1 and calculates the genetic relationship between every possible pair of individuals.
